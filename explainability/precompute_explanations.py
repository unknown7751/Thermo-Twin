"""
Phase 4 — Pre-compute SHAP explanations for the three Thermo-Twin demo scenarios.

Selects a representative fault window for each scenario, runs SHAP GradientExplainer,
and saves results to demo_explanations.json so the dashboard loads instantly.
...
from model.threshold import load_threshold_config, severity_score
...
def main():
    print("=" * 60)
    print("  Thermo-Twin Phase 4: Pre-computing SHAP Demo Explanations")
    print("=" * 60)
    ...
    print("\n[1] Initializing SHAP GradientExplainer")
    print(f"    Background: 150 normal windows from training set")
    ...
    for key, label in SCENARIOS.items():
        mask = y_test == label
        n    = int(mask.sum())
        print(f"[2] {key}  (label='{label}', n={n} windows)")
        ...
        sev = int(severity_score(np.array([err]), threshold, p99_error=p99)[0])

        print(f"    Computing SHAP values...")
        expl = explainer.explain(window)
        expl["severity_score"] = sev
    ...
    print(f"\n[4] Saved -> explainability/demo_explanations.json")
    print(f"    Scenarios saved: {list(results.keys())}")
    print(f"    Load latency:    < 1 second (pre-computed)")

    print("\n" + "=" * 60)
    print("  Phase 4 complete — explanations ready for dashboard.")
    print("=" * 60)
